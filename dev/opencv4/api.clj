...
(defn is-first-with-those-params [m others]
  (let [c-param (count (.getParameterTypes m))
        result (atom true)
...
(def skip-methods ["rectify-3-collinear" "copy-to"])
(def skip-bikkuri-methods ["mean" "norm" "add" "divide" "multiply" "lut" "bilateral-filter"])
...
(defn print-bikkuri-method [methods method-name]
  (let [kebab (->kebab-case method-name)]
    (if (or (in? skip-methods kebab) (in? skip-bikkuri-methods kebab))
      (spit "skip-bikkuri.log" (str kebab "\n") :append true)
      (let [fmethods (filter-m-like methods method-name)
            m (first fmethods)
            all-params (.getParameterTypes m)]
        (if (>= (count all-params) 2)
          (let [params (take 2 all-params)]
            (if
             (and
              ...
              (do
                ...
                (doseq [m fmethods]
                  (let [m-params (.getParameterTypes m)]
                    (if (and (>= (count m-params) 2)
                             (.equals (aget m-params 0) Mat)
                             (.equals (aget m-params 0) (aget m-params 1)))
